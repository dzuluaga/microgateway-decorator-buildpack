...
var startMicro = function(micro_data, callback){
    ...
    const edgemicro_args = [
        'start', 
        '-o', micro_data.required.APIGEE_MICROGATEWAY_ORG,
        '-e', micro_data.required.APIGEE_MICROGATEWAY_ENV,
        '-k', micro_data.required.APIGEE_MICROGATEWAY_KEY,
        '-s', micro_data.required.APIGEE_MICROGATEWAY_SECRET,
        '-c', micro_data.required.APIGEE_MICROGATEWAY_CONFIG_DIR
    ]
    if (micro_data.optional.hasOwnProperty("APIGEE_MICROGATEWAY_PROCESSES").length > 0){
        edgemicro_args = edgemicro_args.concat(['-p', micro_data.optional.APIGEE_MICROGATEWAY_PROCESSES])
    }
